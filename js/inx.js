...
document.getElementById('campaniaModal').addEventListener('change', action =>{
  ...
  .then(data => {
    ...
    if(action.target.value == 6){
      ...
      const inxPuestoT = data.response.findIndex(x => x.Nombre == 'Puesto de trabajo');
      const inxPaqInt = data.response.findIndex(x => x.Nombre == 'Paquete de interés');
      ...
      if(action.target.value == 5){
        let catPuestoT = data.response.filter(x => x.idPadre == data.response[inxPuestoT].id);
        options = '<option hidden value max selected>Seleccione una opción</option>';
        
        catPuestoT.forEach(i => {
          options += `<option attr-id="${i.id}" value="${i.Nombre}">${i.Nombre}</option>`;
        });
  
        document.querySelector('#PuestoTrabajo_select').innerHTML = options;
      }
      
      let catPaqInt = data.response.filter(x => x.idPadre == data.response[inxPaqInt].id);
      options = '<option hidden value max selected>Seleccione una opción</option>';
      ...
      document.querySelector('#PaqIntVL_select').innerHTML = options;
...
document.querySelector('#motivo_select').addEventListener('change', e => {
  ...
  }else if(idCampania == 5 && ![255,256,257].includes(parseInt(e.target.value))){
    document.querySelector('.campoTip').classList.add('d-none');
    document.querySelector('#tip_select').setAttribute('attr-req',true);
    document.querySelector('#tip_select').removeAttribute('required');
...
document.getElementById('formOperativo').addEventListener('submit', f => {
  ...
  if(![2].includes(document.getElementById('idCampania').value) && elTip.value != ''){
